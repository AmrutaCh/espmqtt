...
static esp_err_t esp_mqtt_set_config(esp_mqtt_client_handle_t client, const esp_mqtt_client_config_t *config)
{
    ...
    if (config->client_id) {
        ...
        platform_get_chipid(client->connect_info.client_id);
    }
...
static esp_err_t esp_mqtt_destroy_config(esp_mqtt_client_handle_t client)
{
    ...
    {
        ESP_LOGI(TAG, "mqtt client cfg already destroyed");
    }
    return ESP_OK;
}
...
static esp_err_t esp_mqtt_connect(esp_mqtt_client_handle_t client, int timeout_ms)
{
    ...
    ESP_LOGI(TAG, "Sending MQTT CONNECT message, type: %d, id: %04X", client->mqtt_state.pending_msg_type, client->mqtt_state.pending_msg_id);

    write_len = transport_write(client->transport,
    (char *)client->mqtt_state.outbound_message->data, client->mqtt_state.outbound_message->length, client->config->network_timeout_ms);
    if (write_len < 0) {
        ESP_LOGE(TAG, "Writing failed, errno= %d", errno);
        return ESP_FAIL;
    }
    read_len = transport_read(client->transport, (char *)client->mqtt_state.in_buffer, client->mqtt_state.outbound_message->length, client->config->network_timeout_ms);
    if (read_len < 0) {
...
esp_err_t esp_mqtt_client_destroy(esp_mqtt_client_handle_t client)
{
    if(xHandle_mqttTask)
    {
        vTaskDelete(xHandle_mqttTask);
        printf("\n Task Delete mqtt task");
    }
    ...
    {
        ESP_LOGI(TAG, "mqtt client already destroyed");
    }

    printf("\n esp_mqtt_client_destroy done");
...
{
    int write_len = transport_write(client->transport, (char *)client->mqtt_state.outbound_message->data, client->mqtt_state.outbound_message->length, client->config->network_timeout_ms);
    // client->mqtt_state.pending_msg_type = mqtt_get_type(client->mqtt_state.outbound_message->data);
    if (write_len <= 0) {
...
static void deliver_publish(esp_mqtt_client_handle_t client, uint8_t *message, int length)
{
    ...
    do
    {
        ...
        len_read = transport_read(client->transport, (char *)client->mqtt_state.in_buffer, client->mqtt_state.in_buffer_length, client->config->network_timeout_ms);
        if (len_read <= 0) {
...
{
    ESP_LOGD(TAG, "mqtt_enqueue id: %d, type=%d successful", client->mqtt_state.pending_msg_id, client->mqtt_state.pending_msg_type);
    //lock mutex
    if (client->mqtt_state.pending_msg_count > 0) {
        //Copy to queue buffer
        outbox_enqueue(client->outbox, client->mqtt_state.outbound_message->data, client->mqtt_state.outbound_message->length, client->mqtt_state.pending_msg_id, client->mqtt_state.pending_msg_type, platform_tick_get_ms());
    }
    //unlock
}
...
static void esp_mqtt_task(void *pv)
{
    ...
    while (client->run) {
        ...
        case MQTT_STATE_CONNECTED:
            ...
            if (mqtt_process_receive(client) == ESP_FAIL) {
                ESP_LOGE(TAG, "Error Process Receive");
                esp_mqtt_abort_connection(client);
                break;
            }
    ...
    ESP_LOGD(TAG, "Deleting mqtt Task");
    transport_close(client->transport);
...
int esp_mqtt_client_publish(esp_mqtt_client_handle_t client, const char *topic, const char *data, int len, int qos, int retain)
{
    ...
    client->mqtt_state.outbound_message = mqtt_msg_publish(&client->mqtt_state.mqtt_connection, topic, data, len, qos, retain, &pending_msg_id);
    if (qos > 0) {
        client->mqtt_state.pending_msg_type = mqtt_get_type(client->mqtt_state.outbound_message->data);
